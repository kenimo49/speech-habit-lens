...
```
                           ┌─────────────────────┐
   .wav (60s, 16kHz) ────▶│  AmiVoice 非同期HTTP │
                           │  ?d=-a-general      │
                           │  ?contentId         │──┬─▶ recognized text + timestamps
                           │  +emotion=ESAS      │  └─▶ 20 emotion params × ~30 samples
                           └─────────────────────┘
                                      │
                                      ▼
                           ┌─────────────────────┐
                           │   Claude (3 calls)  │
                           │  ・acoustic_layer   │ ESAS 時系列の特徴抽出
                           │  ・text_layer       │ テキスト構造の癖抽出
                           │  ・cross_layer ⭐   │ 音響×テキストの連動 (差別化の核)
                           └─────────────────────┘
                                      │
                                      ▼
                           ┌─────────────────────┐
...
| `cli.py` | `click` ベースのCLI、`shl analyze` / `shl compare` (v1.0) | `main()` |
...
逆方向の依存は禁止 (`recognize.py` から `report.py` を呼ばない等)。
...
| 拡張 | 触る場所 |
|---|---|
| before/after 比較 | `cli.py` に `shl compare` subcommand、`report.py` に diff renderer |
| HTML/PDF出力 | `report.py` に renderer インタフェース、weasyprint等を別 deps に |
| Web UI (Streamlit) | `webui/` 別パッケージ、内部で `analyze()` を呼ぶ |
